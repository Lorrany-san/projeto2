...
public class ClienteDAO {
    ...
    public void incluir(Cliente c) throws Exception{
        ...
        "INERT INTO CLIENTE (NOME, CPF, NASCIMENTO) VALUES(?, ?, ?)",
                Statement.RETURN_GENERATED_KEYS);
        pst.setString(1, c.getNome());
        pst.setString(2, c.getCpf());
        pst.setDate(3, (java.sql.Date) new Date(c.getNascimento().getTime()));
        pst.executeUpdate();
    ...
    public Vector<Cliente> carregarCombo() throws Exception{
        ...
        "SELECT CODIGOI, NOME FROM CLIENTE ORDER BY NOME");
        Vector<Cliente> v = new Vector<Cliente>();
        while(rs.next())
            v.add(new Cliente(rs.getInt("CODIGO"),
            rs.getString("NOME")));
        return v;
    }
